...
func (e EngineStore) EngineByID(ctx context.Context, id string) (models.Engine, error) {
	...
	err = tx.QueryRowContext(ctx, "SELECT id, displacement, no_of_cylinders, car_range FORM engine WHERE id = $1", id).Scan(
...
func (e EngineStore) EngineUpdate(ctx context.Context, id string, engineReq *models.EngineRequest) (models.Engine, error) {
	...
	results, err := tx.ExecContext(ctx, "UPDATE engine SET displacement = $1, no_of_cyclinder= $2, car_range = $3 WHERE id =$4", engineReq.Displacement, engineReq.NoOfCylinders, engineReq.CarRange, engineID)
